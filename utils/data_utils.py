...
def split_dataset(df, train_split_month=16, test_split_month=20, remain_same=True):
    data = df.copy()
    data["date"] = pd.to_datetime(data["date"])
    training_df = transform_date_column_and_drop_it(
        data[
            data["date"] < data["date"].min() + pd.DateOffset(months=train_split_month)
        ],
        "date",
        remain_same,
    )
    test_df = transform_date_column_and_drop_it(
        data[
            data["date"] > data["date"].min() + pd.DateOffset(months=train_split_month)
        ][data["date"] < data["date"].min() + pd.DateOffset(months=test_split_month)],
        "date",
        remain_same,
    )
...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        input_p_lag = torch.tensor(
            self.df[self.feature_columns]
            .iloc[(idx) : (idx + self.p_lag), :]
            .astype(float)
            .to_numpy()
            .transpose()
            .reshape(1, -1),
        ...
        if self.modelling_task == "univariate":
            target = torch.tensor(
                ...
                .iloc[(idx + self.p_lag) : (idx + self.p_lag + self.future_steps), :]
                .astype(float)
                .to_numpy()
            ).reshape(1, -1)
        else:
            target = torch.tensor(
                self.df.iloc[
                    (idx + self.p_lag) : (idx + self.p_lag + self.future_steps), 0:7
                ]
                .astype(float)
                .to_numpy()
            ).reshape(1, -1)
