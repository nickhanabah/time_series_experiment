...
def plot_multistep_forecast(test_data, neural_net, future_steps, number_of_forecasts= 100): 
    ...
    plt.title(f"Prediction plot for {number_of_forecasts} forecasts of {neural_net.model}, {future_steps} each")
    plt.legend(loc="upper left")
    plt.xlabel("Time Steps")
    plt.ylabel("Oil Temparature (Target variable)")

    plt.savefig(f'/workspaces/time_series_experiment/plots/{neural_net.model}_{future_steps}fs_{number_of_forecasts}fcs_{neural_net.model}plag.png')
    plt.savefig(f'/workspaces/time_series_experiment/plots/{neural_net.model}_{future_steps}fs_{number_of_forecasts}fcs_{neural_net.model}plag.pdf')